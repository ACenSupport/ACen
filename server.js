...
app.post('/update_sidebar', async (req, res) => {
    if (!req.session.user || !req.session.user.is_admin) return res.status(403).send("권한이 없어.");
    const { team_name, reset_logo, logo_base64, sidebar_emoji } = req.body;
    ...
    if (reset_logo === 'yes') {
        ...
        if (logo_base64) {
            updateData.logo_url = logo_base64;
            updateData.emoji = null;
        } else if (sidebar_emoji) {
            updateData.emoji = sidebar_emoji;
            updateData.logo_url = null;
        }
    }
...
app.post('/update_profile', async (req, res) => {
    ...
    if (reset_profile_img === 'yes') {
        ...
        if (profile_img_base64) {
            ...
            req.session.user.emoji = null;
        } else if (profile_emoji) {
            user.emoji = profile_emoji;
            user.profile_img = null;
            req.session.user.emoji = profile_emoji;
            req.session.user.profile_img = null;
        }
    }
...
app.get('/main', async (req, res) => {
    ...
    for (let u of users) {
        ...
        team_status.push({ name: member, status, reason, profile_img, emoji });
    ...
    for (let k in groups) {
        ...
        for (let i = 1; i < arr.length; i++) {
            ...
            if (next.start_date <= nextDayStr || next.start_date === nextBizStr || next.start_date <= current.end_date) {
                if (next.end_date > current.end_date) current.end_date = next.end_date;
...
app.post('/submit', async (req, res) => {
    ...
    if (editIds.length > 0) overlapQuery.id = { $nin: editIds };

    let hasOverlap = await Record.exists(overlapQuery);
    if (hasOverlap) return res.send("<script>alert('해당 일자에 등록된 복무가 있습니다.'); history.back();</script>");

    if (editIds.length > 0) await Record.deleteMany({ id: { $in: editIds } });

    let lastRec = await Record.findOne().sort('-id').lean();
    let nextId = lastRec ? lastRec.id + 1 : 1;

    await Record.create({ id: nextId, name: member, reason, start_date, end_date });
    res.redirect('/calendar' + (current_calendar_date ? '?date=' + current_calendar_date : ''));
});
...
app.get('/delete/:ids', async (req, res) => {
    ...
    if (!req.session.user.is_admin) {
        ...
        if (recordsToDelete.some(r => r.name !== req.session.user.name)) {
...
app.post('/update_leave', async (req, res) => {
    if (!req.session.user || !req.session.user.is_admin) return res.status(403).send("권한이 없어.");
    await User.updateOne({ emp_id: req.body.emp_id }, { leave: parseFloat(req.body.granted) });
...
app.post('/admin/action', async (req, res) => {
    ...
    if (emp_id === '60514' && action === 'delete') return res.send("<script>alert('최고 관리자는 삭제할 수 없습니다.'); history.back();</script>");
    if (action === 'reset') await User.updateOne({ emp_id }, { pw: 'new1234@' });
...
app.post('/admin/restore', async (req, res) => {
    ...
    try {
        ...
        return res.send("<script>alert('파일 처리 중 오류가 발생했습니다.'); history.back();</script>");
    }
});
...
app.listen(5000, '0.0.0.0', () => console.log('Node.js server running on port 5000'));
